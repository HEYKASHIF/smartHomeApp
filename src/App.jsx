...
export default function App() {
  ...
  useEffect(() => {
    save({ users, currentUserId, state });
  }, [users, currentUserId, state]);
      ...
      setState((prev) => {
        ...
        for (const d of devices) {
          if (d.power === "On" && typeof d._lastEnergyTs === "number") {
            ...
            if (ms > 0) {
              const watt = Number.isFinite(d.watt)
                ? d.watt
                : WATT[d.type] || 100;
              d.energyKWh =
                (Number(d.energyKWh) || 0) + (watt / 1000) * (ms / 3600000);
              d._lastEnergyTs = now;
              changed = true;
            }
          }
            ...
            if (now - d._onSince > 24 * 3600000) {
              prev.alerts = [
                ...prev.alerts,
                {
                  id: uid(),
                  ts: now,
                  message: `${d.name} is On for over 24 hours. Consider turning it Off or resetting.`,
                },
              ];
              d._alerted = true;
              changed = true;
        ...
        if (thisMin !== prev._lastRoutineMinute && prev.routines.length) {
          ...
          for (const r of prev.routines) {
            const [rh, rm] = String(r.time || "00:00")
              .split(":")
              .map((x) => Number(x) || 0);
            if (rh === hh && rm === mm) {
              ...
              prev.logs = [
                ...prev.logs,
                {
                  ts: now,
                  type: "routine",
                  message: `Routine '${r.name}' executed`,
                },
              ];
              changed = true;
            }
        ...
        if (!changed && thisMin === prev._lastRoutineMinute)
          return { ...prev, _lastRoutineMinute: thisMin };
  ...
  function removeRoom(id) {
    setState((p) => ({
      ...
      devices: p.devices.filter((d) => d.roomId !== id),
    }));
  }
  ...
  function addDevice(roomId, type, name, rawFeatures = {}, watt) {
    ...
    const safeWattVal = (() => {
      const n = Number(watt);
      return Number.isFinite(n) && n > 0 ? n : WATT[type] || 100;
    })();

    let features = { ...tpl.defaults };
    if (type === "lamp") {
      const b = clamp(
        Number(rawFeatures.brightness || features.brightness),
        0,
        100
      );
    ...
    if (type === "fan") {
      const s = ["Low", "Medium", "High"].includes(rawFeatures.speed)
        ? rawFeatures.speed
        : "Medium";
      features.speed = s;
    }
    if (type === "thermostat") {
      const t = clamp(
        Number(rawFeatures.temperature || features.temperature),
        16,
        32
      );
  ...
  function togglePower(id, to) {
    setState((p) => {
      ...
      p.logs = [
        ...p.logs,
        { ts: now, type: "device", message: `${d.name} -> ${next}` },
      ];
      return { ...p, devices };
    });
  }
  ...
  function setFeature(id, key, value) {
    setState((p) => {
      ...
      if (!dev) return p;
      if (dev.type === "lamp" && key === "brightness")
        dev.features.brightness = clamp(Number(value), 0, 100);
      if (dev.type === "fan" && key === "speed")
        dev.features.speed = ["Low", "Medium", "High"].includes(value)
          ? value
          : "Medium";
      if (dev.type === "thermostat" && key === "temperature")
        dev.features.temperature = clamp(Number(value), 16, 32);
      return { ...p, devices };
    });
  }
  ...
  function addRoutine(name, time, target, targetValue, to) {
    ...
    if (!nm || !/^\d{2}:\d{2}$/.test(tm)) return;
    const t = target === "room" || target === "type" ? target : "all";
    let tv = targetValue;
    if (t === "room" && !state.rooms.find((r) => r.id === tv)) tv = undefined;
    if (t === "type" && !DEVICE_TYPES.find((x) => x.type === tv))
      tv = undefined;
    setState((p) => ({
      ...p,
      routines: [
    ...
    <div className="min-h-screen bg-slate-100 text-slate-900">
      <div className="max-w-5xl mx-auto p-6 space-y-6">
        <header className="flex items-center justify-between">
          <h1 className="text-2xl font-bold">🏠 My SmartHome_Manager</h1>
          <div className="flex items-center gap-2">
            {currentUser ? (
              <>
                <span className="text-sm">{currentUser.name}</span>
                <button
                  className="inline-flex items-center justify-center rounded-lg border px-3 py-2 text-sm bg-slate-50 hover:bg-slate-100"
                  onClick={logout}
                >
                  Logout
                </button>
            ...
            <button
              className="inline-flex items-center justify-center rounded-lg border px-3 py-2 text-sm bg-rose-600 text-white hover:bg-rose-700 border-transparent"
              onClick={resetAll}
            >
              Reset Data
            </button>
        ...
        {tab === "login" ? (
          <form
            onSubmit={(e) => {
              ...
              try {
                onLogin(f.get("email"), f.get("password"));
              } catch (ex) {
                setErr(ex.message);
              }
            }}
          ...
          >
            ...
            <button className="inline-flex items-center justify-center rounded-lg border px-3 py-2 text-sm bg-blue-600 text-white border-transparent hover:bg-blue-700">
              Login
            </button>
          </form>
        ) : (
          <form
            onSubmit={(e) => {
              ...
              const f = new FormData(e.currentTarget);
              try {
                onRegister(f.get("name"), f.get("email"), f.get("password"));
              } catch (ex) {
                setErr(ex.message);
              }
            }}
...
function RoomAdd({ onAdd }) {
  ...
  return (
    <div className="space-y-2">
      <label className="text-xs text-slate-600" htmlFor="roomName">
        Add room
      </label>
      <div className="flex gap-2">
        <input
  ...
  function onCreate(e) {
    ...
    try {
      addDevice(
        ...
        f.get("name"),
        {
          brightness: f.get("feat:brightness"),
          speed: f.get("feat:speed"),
          temperature: f.get("feat:temperature"),
        },
        f.get("watt")
      );
      const t = selType;
      ...
      setError(ex.message || "Failed to create device");
    }
  }
  ...
  return (
    <div className="bg-white border border-slate-200 rounded-2xl shadow">
      ...
      <div className="p-4">
        ...
        {!hasRooms && (
          <div className="text-amber-700 mb-3">Add a room first.</div>
        )}
        ...
        >
          ...
          <div className="sm:col-span-2 md:col-span-3 flex justify-end">
            <button
              className="inline-flex items-center justify-center rounded-lg border px-4 py-2 text-sm bg-blue-600 text-white border-transparent hover:bg-blue-700"
              disabled={!hasRooms}
            >
              Create Device
            </button>
          ...
          {devices.map((d) => (
            <div key={d.id} className="p-3 rounded-xl border border-slate-200">
              <div className="flex items-start justify-between">
                ...
                <select
                  value={d.power}
                  onChange={(e) => togglePower(d.id, e.target.value)}
              ...
              <div className="mt-3 flex gap-2 items-center flex-wrap">
                {d.type === "lamp" && (
                  <div className="flex items-center gap-2 text-sm">
                    ...
                    <input
                      ...
                      min={0}
                      max={100}
                      value={safeNum(d.features.brightness, 50)}
                      onChange={(e) =>
                        setFeature(d.id, "brightness", e.target.value)
                      }
                ...
                {d.type === "fan" && (
                  <div className="flex items-center gap-2 text-sm">
                    ...
                    <select
                      value={
                        ["Low", "Medium", "High"].includes(d.features.speed)
                          ? d.features.speed
                          : "Medium"
                      }
                      onChange={(e) =>
                        setFeature(d.id, "speed", e.target.value)
                      }
                ...
                {d.type === "thermostat" && (
                  <div className="flex items-center gap-2 text-sm">
                    ...
                    <input
                      ...
                      min={16}
                      max={32}
                      value={safeNum(d.features.temperature, 24)}
                      onChange={(e) =>
                        setFeature(d.id, "temperature", e.target.value)
                      }
                      className="border rounded-lg px-2 py-1 w-20"
                    />
                  </div>
                ...
                <span className="ml-auto text-xs text-slate-500">
                  Watt: {Number.isFinite(d.watt) ? d.watt : WATT[d.type] || 100}
                </span>
              </div>
            </div>
          ))}
...
function Dashboard({ rooms, devices, togglePower, setFeature }) {
  return (
    <section className="bg-white border border-slate-200 rounded-2xl shadow">
      <div className="p-4 border-b border-slate-200 flex items-center justify-between">
        <h2 className="text-lg font-semibold">Appliance Dashboard</h2>
        <span className="text-sm text-slate-600">{devices.length} total</span>
      </div>
  ...
  return (
    <section className="bg-white border border-slate-200 rounded-2xl shadow">
      <div className="p-4 border-b border-slate-200 flex items-center justify-between">
        ...
        <span className="text-sm text-slate-600">{routines.length} total</span>
      </div>
      <div className="p-4">
        <form
            ...
            >
              <div>
                ...
                <div className="text-xs text-slate-600">
                  Time: {r.time} • Target: {r.target}{" "}
                  {r.targetValue ? `(${r.targetValue})` : ""} → {r.to}
                </div>
              </div>
              <button
                className="inline-flex items-center justify-center rounded-lg border px-3 py-2 text-sm bg-slate-50 hover:bg-slate-100"
  ...
  return (
    <section className="bg-white border border-slate-200 rounded-2xl shadow">
      <div className="p-4 border-b border-slate-200">
        <h2 className="text-lg font-semibold">Energy Usage (Simulated)</h2>
      </div>
      <div className="p-4">
        <div className="text-sm mb-2">
          Total: <b>{total.toFixed(3)} kWh</b>
        </div>
        <div className="space-y-1 text-sm">
          {devices.map((d) => (
            <div key={d.id} className="flex items-center justify-between">
              <span>{d.name}</span>
              <span>{safeNum(d.energyKWh, 0).toFixed(3)} kWh</span>
            </div>
          ))}
          {devices.length === 0 && (
        ...
        <p className="text-xs text-slate-500 mt-2">
          Tip: Devices consume energy only while <b>On</b>.
        </p>
      </div>
    </section>
  );
...
function coerceDevice(d) {
  ...
  const watt =
    Number.isFinite(d?.watt) && d.watt > 0 ? d.watt : WATT[type] || 100;
  const power = d?.power === "On" || d?.power === "Off" ? d.power : "Off";
  const features = {
    ...(DEVICE_TYPES.find((t) => t.type === type)?.defaults || {}),
    ...(d?.features || {}),
  };
  if (type === "lamp")
    features.brightness = clamp(Number(features.brightness || 50), 0, 100);
  if (type === "fan")
    features.speed = ["Low", "Medium", "High"].includes(features.speed)
      ? features.speed
      : "Medium";
  if (type === "thermostat")
    features.temperature = clamp(Number(features.temperature || 24), 16, 32);
  return {
    id: d.id || uid(),
    roomId: d.roomId,
    type,
    name: String(
      d.name || DEVICE_TYPES.find((t) => t.type === type)?.label || "Device"
    ),
    power,
    features,
    watt,
    energyKWh: safeNum(d.energyKWh, 0),
    _lastEnergyTs: d._lastEnergyTs ?? null,
    _onSince: d._onSince ?? null,
    _alerted: !!d._alerted,
  };
}
function runRoutine(devicesDraft, r) {
  ...
  const pred =
    r.target === "room" && r.targetValue
      ? (d) => d.roomId === r.targetValue
      : r.target === "type" && r.targetValue
      ? (d) => d.type === r.targetValue
      : () => true;
  for (const d of devicesDraft) {
    if (pred(d)) applyPower(d, to);
  }
}
